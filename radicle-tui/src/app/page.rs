...
impl ViewPage for HomeView {
    fn mount(
        ...
        let patch_browser =
            widget::home::patches(theme, &self.patches, &context.profile).to_boxed();
    ...
    fn state(&self) -> PageState {
        PageState::Map(
            [(
                "patches".to_string(),
                PageStateValue::Patches(self.patches.clone(), self.selected_patch),
            )]
            .iter()
            .cloned()
            .collect(),
...
pub enum PageStateValue {
    /// List of patches and its selected index
