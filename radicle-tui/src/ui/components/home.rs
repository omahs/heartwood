...
use crate::ui::cob::{ItemId, PatchItem};
use crate::ui::layout;
...
pub struct PatchBrowser {
    table: Widget<Table<ItemId, PatchItem>>,
    shortcuts: Widget<Shortcuts>,
}
...
impl PatchBrowser {
    pub fn new(theme: &Theme, profile: &Profile, id: &Id, shortcuts: Widget<Shortcuts>) -> Self {
        ...
        let mut model = TableModel::default()
            .with_column(label(" ● "), ColumnWidth::Fixed(3))
            .with_column(label("ID"), ColumnWidth::Fixed(7))
            .with_column(label("Title"), ColumnWidth::Grow)
            .with_column(label("Author"), ColumnWidth::Fixed(21))
            .with_column(label("Head"), ColumnWidth::Fixed(7))
            .with_column(label("+"), ColumnWidth::Fixed(4))
            .with_column(label("-"), ColumnWidth::Fixed(4))
            .with_column(label("Opened"), ColumnWidth::Fixed(18));
        ...
        if let Ok(all) = patches.all() {
            ...
            for (id, patch, _) in patches {
                if let Ok(item) = PatchItem::try_from((profile, &repo, id, patch)) {
                    model.push_item((ItemId::Patch(id), item));
                }
            }
        }
